...
class UI:
    ...
    def draw_player_panels(self, screen, game):
        ...
        if game.player1_type == 'solid':
            type_text = self.font_tiny.render("ОДНОЦВЕТНЫЕ", True, (100, 255, 100))
        elif game.player1_type == 'stripe':
            type_text = self.font_tiny.render("ПОЛОСАТЫЕ", True, (100, 255, 100))
        else:
        ...
        if game.player2_type == 'solid':
            type_text2 = self.font_tiny.render("ОДНОЦВЕТНЫЕ", True, (100, 255, 100))
        elif game.player2_type == 'stripe':
            type_text2 = self.font_tiny.render("ПОЛОСАТЫЕ", True, (100, 255, 100))
        else:
        ...
        # Если был фол, добавляем значок
        foul_text = " ⚠️" if game.pending_foul else ""
        turn_text = f"ХОДИТ: {current_name}{foul_text}"
        turn_x = SCREEN_WIDTH // 2 - 67
        turn_y = 8
        
        if game.current_player != self.last_player:
            self.last_player = game.current_player
            self.cached_turn_text = turn_text
            if game.pending_foul:
                self.cached_turn_surface = self.font_small.render(turn_text, True, (255, 100, 100))
            else:
                self.cached_turn_surface = self.font_small.render(turn_text, True, COLORS['GOLD'])
        
        bg_rect = pygame.Rect(turn_x - 67, turn_y - 2, 300, 32)
        ...
        timer_x = SCREEN_WIDTH - 100
        timer_y = 10
        
        if timer_text != self.last_timer_value:
            self.last_timer_value = timer_text
            self.cached_timer_surface = self.font_small.render(timer_text, True, COLORS['WHITE'])
        ...
        if self.cached_timer_surface:
            screen.blit(self.cached_timer_surface, (timer_x, timer_y))
    ...
    def draw_instructions(self, screen):
        ...
        for i, line in enumerate(instr):
            text = self.font_tiny.render(line, True, (100, 100, 100))
            x = SCREEN_WIDTH // 2 - text.get_width() // 2
